...
class VRPTW:
    ...
    def fill(self, path):
        ...
        self.visited = [i for i in range(1, len(self.shops))]
            ...
            while best_shop != 0:
                ...
                for id in self.visited:
                    ...
                    if id > self.vehicles[-1].path[-1]:
                        i, j = j, i
                    if ( self.shops[id].w_start <= self.vehicles[-1].time + self.distances[i][j - i - 1] <= self.shops[id].w_end and
